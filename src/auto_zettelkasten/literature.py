...
CAUSAL_LANGUAGE = re.compile(
    r"\b(?:caus(?:e|es|ed|al|ality|ation)|effect|(?:lead(?:s|ing)?|led) to|produces?|drives?|result(?:s|ed|ing)? in|"
    r"improv(?:e|es|ed)|enhanc(?:e|es|ed)|increas(?:e|es|ed)|reduc(?:e|es|ed)|"
    r"rais(?:e|es|ed)|"
    r"undermin(?:e|es|ed)|hinder(?:s|ed)?|prevent(?:s|ed)?|"
    r"facilitat(?:e|es|ed)|forg(?:e|es|ed)|helps?(?:\s+to)?|work(?:s|ed)? best|succeeds? when)\b",
    re.I,
)
...
ATTRIBUTED_ARGUMENT = re.compile(
    r"\b(?:assert(?:s|ed)?|claim(?:s|ed)?|argu(?:e|es|ed)|recommend(?:s|ed)?|"
    r"advocat(?:e|es|ed)|propos(?:e|es|ed)\s+that|guidance|converg(?:e|es|ed|ing)\s+"
    r"on\s+(?:the\s+)?proposition|(?:source|cited sources)\s+describe(?:s)?\s+the\s+"
    r"relationship\s+this\s+way)\b",
    re.I,
)
...
NONCAUSAL_STATISTICAL_FRAME = re.compile(
    r"\b(?:is|are|was|were)?\s*(?:associated|correlated|linked)\s+with\s+"
    r"(?:an?\s+)?(?:higher|lower|increased|reduced)\b|"
    r"\breported\s+(?:(?:marginal|statistical|estimated)\s+)?effect\b",
    re.I,
)
CAUSAL_NEGATION = re.compile(
    ...
    r"(?:lead(?:s|ing)?|led)\s+to|drives?|result(?:s|ed|ing)?\s+in|improv(?:e|es|ed)|increas(?:e|es|ed)|"
    r"reduc(?:e|es|ed))\b|"
...
def _has_unqualified_causal_language(value: Any) -> bool:
    text = re.sub(
        r"\broot causes?\b",
        "underlying conflict conditions",
        str(value or ""),
        flags=re.I,
    )
    text = re.sub(
        r"\bfacilitat(?:e|es|ed|ing)\s+(?:direct\s+)?communication\b",
    ...
    clauses = re.split(
        ...
        r"\band\s+(?=(?:caus(?:e|es|ed)|(?:lead(?:s|ing)?|led)\s+to|produces?|drives?|result(?:s|ed|ing)?\s+in|"
        r"improv(?:e|es|ed)|enhanc(?:e|es|ed)|increas(?:e|es|ed)|reduc(?:e|es|ed)|"
        r"rais(?:e|es|ed)|"
        r"undermin(?:e|es|ed)|hinder(?:s|ed)?|prevent(?:s|ed)?|"
        r"facilitat(?:e|es|ed)|forg(?:e|es|ed)|helps?(?:\s+to)?|work(?:s|ed)?\s+best|"
        r"succeeds?\s+when|makes?)\b)",
        text,
        flags=re.I,
    )
    for sentence in clauses:
        causal_text = NONCAUSAL_STATISTICAL_FRAME.sub("", sentence)
        causal_comparison = re.search(
            r"\b(?:more|less)(?:\s+\w+){0,3}\s+(?:effective|successful)\b|"
            r"\bmakes?\b[^.!?]{0,80}\b(?:(?:more|less)\s+(?:durable|lasting|stable|sustainable|successful|effective)|better|worse)\b",
            causal_text,
            flags=re.I,
        )
        if not CAUSAL_LANGUAGE.search(causal_text) and not causal_comparison:
            continue
        ...
        if (
            ATTRIBUTED_ARGUMENT.search(causal_text)
            or CAUSAL_NEGATION.search(causal_text)
        ):
            continue
        return True
    return False
...
def _anchor_supports_causal_claim(anchor: Mapping[str, Any]) -> bool:
    ...
    return not bool(
        re.search(
            ...
            r"\b(?:cannot|can not|can't)\s+rule out\b|"
            r"\bcounterfactual\b[^.!?;]{0,80}\b(?:not observed|not tested|unobserved)\b",
            restrictions,
            flags=re.I,
        )
    )
...
def _narrow_noncausal_organizational_language(
    value: Any, *, boundary: str = "", _append_boundary: bool = True
) -> str:
    """Render a non-causal result as an observed comparison or association."""
    ...
    if len(sentences) > 1:
        has_explicit_boundary = any(
            CAUSAL_NEGATION.search(sentence) for sentence in sentences
        )
        return " ".join(
            _narrow_noncausal_organizational_language(
                sentence,
                boundary=boundary,
                _append_boundary=not has_explicit_boundary,
            )
            for sentence in sentences
        )
    source_label = (
        "The cited sources"
        if re.match(r"^(?:The cited sources|The studies)\b", text, flags=re.I)
        else "The source"
    )
    source_verb = "report" if source_label == "The cited sources" else "reports"
    text = re.sub(
        r"^(?:The cited sources?|The studies) report that\s+",
        "",
        text,
        flags=re.I,
    )
    text = re.sub(
        r"\s+This evidence does not by itself establish causation\.?$",
        "",
        text,
        flags=re.I,
    ).strip()
    ...
    if not _has_unqualified_causal_language(text):
        return text.rstrip(" .") + "."
    boundary_text = re.sub(r"\s+", " ", boundary).strip().rstrip(".") or (
        "The study design does not establish that the first factor caused the outcome"
    )
    directive_probability = re.fullmatch(
        r"Using\s+(.+?)\s+increases\s+the probability of\s+(.+?)\s+by\s+"
        r"(about\s+\d+(?:\.\d+)?\s+percentage points?)\s+compared to\s+(.+?)\.?",
        text,
        flags=re.I,
    )
    if directive_probability:
        strategy, outcome, magnitude, comparison = directive_probability.groups()
        return (
            f"In the observed data, cases using {strategy} had a probability of {outcome} "
            f"{magnitude} higher than cases using {comparison}. {boundary_text}."
        )
    unit_probability = re.fullmatch(
        r"Each additional unit of\s+(.+?)\s+raises\s+(.+?)\s+by\s+"
        r"(about\s+\d+(?:\.\d+)?\s+percentage points?)\.?",
        text,
        flags=re.I,
    )
    if unit_probability:
        measure, outcome, magnitude = unit_probability.groups()
        return (
            f"Each additional unit of {measure} was associated with an observed {outcome} "
            f"{magnitude} higher. {boundary_text}."
        )
    twice_as_successful = re.fullmatch(
        r"(.+?)\s+are more than twice as successful as\s+(.+?)\.?",
        text,
        flags=re.I,
    )
    if twice_as_successful:
        first, second = twice_as_successful.groups()
        return (
            f"The observed success rate for {first} was more than twice the rate for {second}. "
            f"{boundary_text}."
        )
    success_rates = re.fullmatch(
        r"(.+?)\s+achieve higher success\s+\((\d+(?:\.\d+)?%)\)\s+than\s+"
        r"(.+?)\s+\((\d+(?:\.\d+)?%)\)\.?",
        text,
        flags=re.I,
    )
    if success_rates:
        first, first_rate, second, second_rate = success_rates.groups()
        return (
            f"The observed success rate was {first_rate} for {first} and {second_rate} for {second}. "
            f"{boundary_text}."
        )
    positive_relationship = re.fullmatch(
        r"(.+?)\s+(?:increases?|raises?|improves?|enhances?)\s+(.+?)\.?",
        text,
        flags=re.I,
    )
    if positive_relationship and not re.search(
        r",\s*(?:but|while|whereas|although|yet)\b",
        positive_relationship.group(1),
        flags=re.I,
    ) and not CAUSAL_NEGATION.search(positive_relationship.group(1)):
        factor, outcome = positive_relationship.groups()
        result = (
            f"{source_label} {source_verb} a positive association between {factor} and "
            f"{outcome}."
        )
        if _append_boundary:
            result += f" {boundary_text}."
        return result
    quoted = text.rstrip(" .")
    result = f"{source_label} {'describe' if source_label == 'The cited sources' else 'describes'} the relationship this way: “{quoted}.”"
    if _append_boundary:
        result += f" {boundary_text}."
    return result
...
def _attribute_practitioner_guidance(value: Any) -> str:
    ...
    text = re.sub(
        r"^Including more groups makes peace more lasting\.?$",
    ...
    if (
        text
        and _has_unqualified_causal_language(text)
        and not re.search(r"\b(?:guidance|guide|manual)\b", text, flags=re.I)
    ):
        text = "The guidance presents this as a practice judgment: " + text
    return text


def _attribute_source_argument(value: Any) -> str:
    """Preserve a source's theoretical or interpretive claim as an argument."""

    text = re.sub(r"\s+", " ", str(value or "")).strip()
    if not text or not _has_unqualified_causal_language(text):
        return text
    if re.search(
        r"\b(?:source|article|paper|author|study|analysis|theory)\b[^.!?]{0,80}"
        r"\b(?:argu(?:e|es|ed)|propos(?:e|es|ed)|specif(?:y|ies|ied|ying)|"
        r"theori[sz](?:e|es|ed)|posits?|presents?)\b",
        text,
        flags=re.I,
    ):
        return text
    if text.startswith(("The ", "This ", "In ", "When ", "If ")):
        text = text[:1].lower() + text[1:]
    return f"The source argues that {text}"


def _add_case_period_context(value: Any, profile: Mapping[str, Any]) -> str:
    """Add a profile's matching dated case period when prose omits it."""

    text = re.sub(r"\s+", " ", str(value or "")).strip()
    if not text:
        return text
    periods = profile.get("periods", []) or _as_mapping(
        profile.get("dimensions")
    ).get("period", [])
    for raw_period in periods or []:
        match = re.fullmatch(
            r"\s*((?:18|19|20)\d{2})\s*[-–—]\s*((?:18|19|20)\d{2})\s*\(([^)]+)\)\s*",
            str(raw_period or ""),
        )
        if not match:
            continue
        start, end, case = match.groups()
        if case.casefold() not in text.casefold() or start in text:
            continue
        case_pattern = re.escape(case)
        replaced = re.sub(
            rf"^In\s+{case_pattern}\s*,\s*",
            f"In the source's {start}–{end} {case} case, ",
            text,
            count=1,
            flags=re.I,
        )
...
def _anchor_is_source_argument(anchor: Mapping[str, Any]) -> bool:
    envelope = _as_mapping(anchor.get("support_envelope"))
    return str(envelope.get("argument_role") or "") in {
        "conceptual",
        "interpretive",
        "normative",
        "methodological",
...
def _semantic_literature_policy(policy: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        if str(key) not in nonsemantic
    }


def _provider_worker_count(request: Any, ready_jobs: int) -> int:
...
class _CheckpointedReasonerCalls:
    ...
    def __init__(
        ...
        *,
        ...
        self.retry_terminal_failures = bool(
        ...
        if not usage:
            usage = _legacy_synthesis_usage(self.root, max_calls=self.max_calls)
            self.cumulative_provider_calls = int(
                usage.get("provider_call_count", 0) or 0
            )
    ...
    def __call__(
        ...
        dependency = {
            ...
            "policy": _semantic_literature_policy(
                _as_mapping(_as_mapping(self.request).get("literature_policy"))
            ...
            if validated_checkpoint_response is None:
                ...
                matching_checkpoint = {
                    **dict(matching_checkpoint),
                    "response": validated_checkpoint_response,
                }
        if matching_checkpoint is not None and isinstance(
            matching_checkpoint.get("response"), Mapping
        ):
            ...
            ) or dict(existing.get("response") or {}) != dict(
        ...
        durable_attempt_count = max(
            (
                int(row.get("attempt", 0) or 0)
                for row in matching_usage_attempts
            ),
            default=0,
        )
        if (
            isinstance(prior_failure, Mapping)
            and bool(prior_failure.get("terminal"))
            and (
                not self.retry_terminal_failures
        ...
        if packet_chars > context_char_budget:
            ...
            failure_payload = {
                ...
                "dependency_context_item_hashes": dependency_context_item_hashes,
                "error": {
                    "type": "LiteratureSynthesisPartialError",
                    "message": (
        ...
        with self._state_lock:
            ...
            budget_group = _synthesis_stage_budget_group(stage, key)
            if not self._stage_budget_available(budget_group):
                raise LiteratureSynthesisPartialError(
                    f"literature_synthesis_stage_budget_reached:{budget_group}"
                )
        ...
        raw_response_for_failure: Mapping[str, Any] | None = None
        try:
            ...
            raw_response = raw_response_for_failure or getattr(
                exc, "raw_response", None
            )
            failure_payload = {
                "checkpoint_schema_version": "1",
                "fingerprint": fingerprint,
    ...
    def _record_checkpoint_hit(self) -> None:
        with self._state_lock:
    ...
    def _record_provider_attempt(
        ...
        with self._state_lock:
            attempts = {
                ...
                for row in self._usage.get("attempts", []) or []
                if isinstance(row, Mapping) and row.get("attempt_id")
            }
            attempts[attempt_id] = dict(event)
            stage_call_counts = Counter(
    ...
    def _stage_budget_available(self, group: str) -> bool:
        del group
        # v0.14 has one cumulative ceiling. Fixed per-stage reservations made
        # valid full-note clusters fail even when the run still had calls.
        return True
    ...
    def _finish_provider_attempt(
        ...
        *,
        ...
        with self._state_lock:
            ...
            row = attempts.get(attempt_id, {"attempt_id": attempt_id})
            row.update(status=status, completed_at=now_iso())
            if failure_class:
    ...
    def _progress(self, stage: str, path: Path, *, active: bool) -> None:
        ...
        values = {
            ...
            "synthesis_call_count": self.cumulative_provider_calls,
            "synthesis_new_call_count": self.provider_calls,
...
def _legacy_synthesis_usage(root: Path, *, max_calls: int) -> dict[str, Any]:
    ...
    for path in sorted(root.rglob("*.yml")) if root.is_dir() else []:
        if path.name == "provider_usage.yml":
            continue
        checkpoint = read_yaml(path, {}) or {}
        if not isinstance(checkpoint, Mapping):
            continue
        if checkpoint.get("deterministic_preflight"):
            continue
        fingerprint = str(checkpoint.get("fingerprint") or "")
        stage = str(checkpoint.get("stage") or "")
        key = str(checkpoint.get("key") or "")
        if not fingerprint or not stage:
            continue
        attempt_id = _stable_hash(
            {
                "legacy": True,
        ...
        attempts[attempt_id] = {
            ...
            "key": key,
            "fingerprint": fingerprint,
            "attempt": max(1, int(checkpoint.get("attempt_count", 1) or 1)),
            "status": str(checkpoint.get("status") or "historical"),
            "migrated_from_legacy_checkpoint": True,
        }
    stage_call_counts = Counter(
        str(row["stage"]) for row in attempts.values()
    )
    return {
        "usage_schema_version": "2",
        "max_calls": max_calls,
...
def _synthesis_stage_budget_group(stage: str, key: str = "") -> str:
    if stage in {
    ...
    if stage == "relationship_verification_escalation":
...
def _revalidate_raw_synthesis_response(
    ...
    relationship_kind = {
        ...
        "relationship_bridge_shard_selection": "shard_pair_selection",
        "relationship_candidate_selection": "candidate_selection",
        "relationship_bridge_candidate_selection": "candidate_selection",
        "relationship_adjudication": "relationship_adjudication",
        "relationship_verification": "relationship_verification",
        "relationship_verification_escalation": "relationship_verification",
    }.get(stage)
    ...
    *,
    ...
    visible_context_components = (
        (
            ...
            "catalogue_shard_ids",
            "existing_family_ids",
            "incremental_source_delta",
            "prior_active_cluster_family_cards",
            "prior_family_state_hash",
        )
        if stage == "cluster_plan"
        else (
            "propositions",
...
def _proposal_family_relations(
    ...
    for raw in proposal.get("family_relations", []) or []:
        ...
        if not set(source_ids) <= covered:
            continue
        if relation_type == "shared_research_problem" and all(
            str(proposal.get(field_name) or "").strip()
            for field_name in (
                "shared_question",
                "bounded_object",
                "coherence_rationale",
            )
        ) and str(raw.get("rationale") or "").strip():
            # The collection reasoner has the complete profiles and is responsible
            # for the analytical judgment that these sources form one research
            # conversation. Admission only checks that every named source and
            # locator-backed anchor is real. Token-overlap scoring previously
            # rejected obvious literatures (internationalized civil wars, conflict
            # prevention, natural resources, and local mediation) because authors
            # used different vocabulary for the same bounded problem.
            relations.append(
                {
                    ...
                    "evidence": evidence,
                    "comparability": {
                        "provider_assessment": dict(
                            _as_mapping(raw.get("comparability"))
                        ),
                        "passed": True,
                        "shared_object_passed": True,
                        "type_specific_passed": True,
                        "explanation": (
                            "The collection reasoner identified one bounded research "
                            "conversation and supplied a valid source-local anchor for "
...
def _numeric_tokens(value: Any) -> set[str]:
    ...
    for match in re.finditer(
        ...
        if (
            ...
            # Publication and case years are context, not numerical findings.
            # They should not make an otherwise supported sentence fail simply
            # because the evidence anchor omits an author-date citation.
            continue
        tokens.add(token)
    return tokens


def _asserts_verified_independence(value: Any) -> bool:
    """Detect affirmative independence claims without rejecting stated limits."""

    text = str(value or "")
    for match in re.finditer(r"\bindependent\b", text, flags=re.I):
        prefix = text[max(0, match.start() - 48) : match.start()]
        if re.search(
            r"\b(?:not|no|lack(?:s|ed|ing)?|without|cannot|can't|uncertain(?:ly)?|"
            r"rather than|not fully)\b[^.!?;]{0,28}$",
            prefix,
            flags=re.I,
        ):
            continue
        ...
        if re.search(
            r"^\s+(?:confirmation|replication|evidence|study|studies|dataset|datasets|data|"
            r"estimate|estimates|source|sources|support|test|tests)\b",
            suffix,
            flags=re.I,
        ):
            return True
...
def _thread_relationship_type(item: Mapping[str, Any]) -> str:
    """Normalize a plain-English thread relationship to its compact machine type."""

    relationship = str(item.get("relationship") or "").strip()
    relationship_prefix = re.split(
        r"\s*[:;—]\s*|,\s*", relationship, maxsplit=1
    )[0]
    normalized = slugify(
        item.get("relationship_type") or relationship_prefix
    ).replace("-", "_")
    aliases = {
        "complementary_alignment": "complementary",
        "complementary_evidence": "complementary",
        "within_program_consistency": "complementary",
        "within_program_alignment": "complementary",
        "conditional_evidence": "conditional",
        "methodological_difference": "methodological",
        "interpretive_difference": "interpretive",
...
def _reconcile_evidence_thread_support(
    ...
    supplied_evidence = [dict(reference) for reference in evidence]
    supplied_anchors = []
    for reference in supplied_evidence:
        ...
        anchor = next(
            (
                row
                for row in profile_by_source.get(source_id, {}).get("claims", []) or []
                if str(row.get("evidence_anchor_id") or row.get("claim_id") or "")
                == anchor_id
            ),
            {},
        )
        if not anchor or not _anchor_is_synthesis_eligible(anchor):
            supplied_anchors = []
            break
        supplied_anchors.append(anchor)
    supplied_numbers = {
        number
    ...
    statement_numeric_literals = set(
        re.findall(r"\b\d[\d,.]*\b", statement)
    )
    supplied_numeric_literals = {
        token
        for anchor in supplied_anchors
    ...
    if (
        statement
        and supplied_evidence
        and len(supplied_anchors) == len(supplied_evidence)
        and statement_numbers.issubset(supplied_numbers)
        and statement_numeric_literals.issubset(supplied_numeric_literals)
    ):
        # A valid collection-reasoner citation is authoritative for qualitative
        # synthesis. Keep only the cheap failure check that every explicit number
        # appears in at least one cited source anchor. Lexical-overlap repair is a
        # fallback for malformed or numerically unsupported citations, not a second
        # semantic judge over well-formed reasoner output.
        _replace_cluster_item_statement(item, statement)
        item["source_ids"] = sorted(
            {str(reference.get("source_id") or "") for reference in supplied_evidence}
        )
        return statement, supplied_evidence
...
def _evidence_ref(claim: Mapping[str, Any]) -> dict[str, Any]:
    ...
    envelope = _as_mapping(claim.get("support_envelope"))
    support_status = str(
    ...
    uncertainty = str(claim.get("uncertainty") or "").strip()
    if support_status != "supported" and uncertainty and uncertainty not in restrictions:
        restrictions.append(uncertainty)
    return {
        "evidence_anchor_id": anchor_id,
        "claim_id": anchor_id,
...
def _sanitize_reasoned_items(
    ...
    *,
    ...
    allow_limited_support: bool = False,
) -> list[dict[str, Any]]:
    ...
    for value in values or []:
        ...
        allowed_support = {"supported"}
        if allow_limited_support:
            allowed_support.add("limited")
        evidence = [
            reference
            for reference in evidence
...
def _universal_claim_missing_core_coverage(
    ...
    if re.search(r"\bboth\b", text, re.I) and not re.search(
        r"\b(?:all|each|every|consistently)\b|"
        r"\bthe literature\s+(?:agrees|shows|finds|reports|establishes|demonstrates)\b",
        text,
        re.I,
    ):
        return len(evidence_source_ids) < 2
    ...
    for source_id in cluster.get("source_ids", []) or []:
        ...
        for anchor in selected:
            ...
            if (
                ...
                if _anchor_is_source_argument(anchor):
                    finding = _attribute_source_argument(finding)
                    plain_english_meaning = _attribute_source_argument(
                        plain_english_meaning
                    )
                else:
...
def validate_streamlined_cluster_synthesis(
    ...
    def locator_match_score(reference: Any, candidate: Any) -> int:
        supplied = _normalized_locator(reference)
        existing = _normalized_locator(candidate)
        if not supplied or not existing:
            return 0
        if supplied == existing:
            return 3
        supplied_parts = [
            part.strip() for part in supplied.split(";") if part.strip()
        ]
        existing_parts = [
            part.strip() for part in existing.split(";") if part.strip()
        ]
        if set(supplied_parts) & set(existing_parts):
            return 2
        for left in supplied_parts:
            for right in existing_parts:
    ...
    def resolve_reference(
        reference: Mapping[str, Any], source_id: str
    ) -> dict[str, Any] | None:
        candidates = claims_by_source.get(source_id, [])
        anchor_id = str(
            reference.get("evidence_anchor_id")
            or reference.get("claim_id")
            or ""
        ...
        if exact is not None:
            return _evidence_ref(exact)
        ranked = sorted(
            (
                (
                    locator_match_score(
                        reference.get("locator"), claim.get("locator")
                    ),
                    _nonnegative_int(claim.get("salience_priority")),
                    str(
                        claim.get("evidence_anchor_id")
                        or claim.get("claim_id")
                        or ""
    ...
    for line_index, line in enumerate(response.get("lines_of_inquiry", []) or []):
        if not isinstance(line, Mapping):
            errors.append("line_of_inquiry_not_mapping")
            continue
        if not str(line.get("title") or "").strip():
            errors.append("line_of_inquiry_requires_title")
        if not str(line.get("synthesis") or "").strip():
            errors.append("line_of_inquiry_requires_synthesis")
        line_id = f"line-{line_index + 1}"
        line_source_ids: list[str] = []
        line_evidence: list[dict[str, Any]] = []
        for finding_index, finding in enumerate(
            line.get("study_findings", []) or []
        ):
            if not isinstance(finding, Mapping):
                errors.append("study_finding_not_mapping")
                continue
            ...
            for raw_reference in raw_evidence:
                if isinstance(raw_reference, str):
                    raw_reference = {
                        "source_id": source_id,
                        "locator": raw_reference,
                    }
                if not isinstance(raw_reference, Mapping):
                    continue
                reference = dict(raw_reference)
                evidence_source = str(
                    reference.get("source_id") or source_id
                )
                if evidence_source != source_id:
                    errors.append("study_finding_evidence_source_mismatch")
                    continue
                resolved = resolve_reference(reference, source_id)
                if resolved is None:
                    warnings.append("study_finding_evidence_unresolved")
                    continue
            ...
            source_contributions.append(
                {
                    "contribution_id": (
                        f"{cluster_id}-{line_index + 1}-{finding_index + 1}"
                    ),
                    "source_id": source_id,
                    "cluster_role": "core",
                    "contribution_kind": "unique_cluster_relevant_finding",
                    "related_proposition_ids": [],
                    "evidence_thread_id": line_id,
                    "finding": str(finding.get("finding") or ""),
                    "technical_result": str(
                        finding.get("technical_result")
                        or finding.get("method_scope")
                        or ""
                    ),
                    "plain_english_meaning": str(
                        finding.get("plain_english_meaning")
                        or finding.get("finding")
                        or ""
                    ),
                    "relation_to_cluster_question": str(
                        finding.get("relation_to_line") or ""
                    ),
                    "comparison_status": "single_source",
        ...
        evidence_threads.append(
            {
                ...
                "question": "",
                "summary": str(line.get("synthesis") or ""),
                "plain_english_meaning": str(line.get("synthesis") or ""),
                "relationship": "",
                "source_ids": sorted(set(line_source_ids)),
                "proposition_ids": [],
    ...
    return {
        **dict(response),
        ...
        "synthesis": str(response.get("bottom_line") or ""),
        "evidence_threads": evidence_threads,
        "source_contributions": source_contributions,
        "supporting_evidence": supporting_evidence,
        "central_findings": [
            {
                "finding": str(line.get("synthesis") or ""),
                "evidence": [
                    evidence
                    for contribution in source_contributions
                    if contribution["evidence_thread_id"]
                    == f"line-{index + 1}"
                    for evidence in contribution.get("evidence", [])
                ],
            }
            for index, line in enumerate(
                response.get("lines_of_inquiry", []) or []
            )
            if isinstance(line, Mapping) and str(line.get("synthesis") or "")
        ],
        "quality_errors": sorted(set(errors)),
        "quality_warnings": sorted(set(warnings)),
        "quality_status": "complete" if not errors else "partial",
        "status": "reasoned" if not errors else "partial",
        "parked_for_review": bool(errors),
    }
    ...
    *,
    ...
    raw_sections = {
        key: _sanitize_reasoned_items(
            ...
            profile_by_source,
            allowed_source_ids=allowed_source_ids,
            allow_limited_support=key == "source_contributions",
        )
    ...
    for section, items in raw_sections.items():
        for item in items:
            ...
            if section == "evidence_threads" and len(
                ...
                thread_relationship_type = _thread_relationship_type(item)
            ...
            if rejection_reason:
                ...
                statement = _add_case_period_context(
                    statement,
                    profile_by_source.get(contribution_source_id, {}),
                )
                _replace_cluster_item_statement(item, statement)
                ...
                if (
                    ...
                    if any(
                        _anchor_is_source_argument(anchor)
                        for anchor in referenced_anchors
                    ):
                        statement = _attribute_source_argument(statement)
                        narrowed_plain_english = _attribute_source_argument(
                            plain_english
                        )
                    else:
                    ...
                    _replace_cluster_item_statement(item, statement)
                    item["plain_english_meaning"] = narrowed_plain_english
            ...
            elif _asserts_verified_independence(statement) and any(
                ...
                _has_unqualified_causal_language(statement)
                or _has_unqualified_causal_language(
                    item.get("plain_english_meaning")
                    or item.get("plain_english")
                    or ""
                )
            ) and not any(
                _anchor_supports_causal_claim(
                ...
                if section in {
                    ...
                    statement = (
                        _attribute_source_argument(statement)
                        if source_argument
                        else _narrow_noncausal_organizational_language(
                            statement, boundary=boundary
                        )
                    )
                    _replace_cluster_item_statement(item, statement)
...
def _reconcile_final_unclustered_sources(
    ...
    """Return the current eligible sources without an active cluster."""

    del prior_unclustered, rejected_clusters
    ...
    for profile in profiles:
        ...
        if (
            not source_id
            or source_id in clustered_source_ids
            or not profile.get("analytical")
            or profile.get("limited")
            or str(profile.get("evidence_eligibility") or "")
            not in {"", "substantive_bounded"}
        ):
            continue
        reconciled.append(
            {
...
def _proposition_debate_state(
    ...
    if len(reported) >= 2:
        ...
        # Opposite coefficient signs are not necessarily opposing findings.
        # "Higher fatalities reduce success" and "low fatalities increase
        # success" describe the same relationship from inverse reference
        # points. A strict quantitative contradiction therefore requires an
        # explicitly normalized predictor and outcome orientation.
        return "mixed_evidence", {
...
def _strict_claim_adjudications(
    *,
    ...
    def assessment_has_opposing_positions(
        ...
        return explicit_argument_opposition
...
def _safe_gap_resolution_text(value: Any, *, fallback: Any = "") -> str:
    """Remove unsupported project-design details from public gap projections."""
    ...
    if re.search(
        ...
        r"natural experiment|large[- ]n|panel data|identification strategy|"
        r"exogenous variation)\b",
        text,
        flags=re.I,
    ):
        return _human_projection_text(fallback)
    return text
...
def _resolution_path_summary(resolution: Mapping[str, Any]) -> list[str]:
    ...
    lines = [
        ...
        "**Evidence needed:** "
        + _safe_gap_resolution_text(
            resolution.get("evidence_needed") or "",
            fallback=(
                "Evidence that distinguishes the mapped relationship from selection "
                "and the competing explanations identified above."
            ),
        ),
    ]
...
def _source_notes_with_custody_relations(
    ...
    """Merge exact custody and matched citation relations into graph metadata."""
    ...
    def add_relation(source_id: str, target_id: str, predicate: str) -> None:
        source = by_source.get(source_id)
        if source is None or target_id not in by_source or target_id == source_id:
            return
        ...
        relations[predicate] = sorted(str(value) for value in target_list)
    ...
    if registry.is_file():
        ...
        for relation in relation_rows:
            relation_type = str(
                relation.get("relation_type") or "zotero_related"
            )
            add_relation(
                str(relation.get("source_id") or ""),
                str(relation.get("related_source_id") or ""),
                (
                    relation_type
...
def _coverage_profile_projection(profile: Mapping[str, Any]) -> dict[str, Any]:
    ...
    anchors = (
        profile.get("claims")
        or profile.get("evidence_anchors")
    ...
    compact_anchors = [
        {
            ...
            "claim_id": str(
                anchor.get("claim_id")
                or anchor.get("evidence_anchor_id")
                or ""
            ),
            "claim": str(
                anchor.get("text") or anchor.get("claim") or ""
            ...
            "support_envelope": {
                key: _as_mapping(anchor.get("support_envelope")).get(key)
                for key in (
                    "argument_role",
                    "coverage",
                    "empirical_role",
                ...
                if _as_mapping(anchor.get("support_envelope")).get(key)
            },
        }
        for anchor in anchors
        if isinstance(anchor, Mapping)
    ...
    return {
        ...
        "note_id": profile.get("note_id"),
        "title": str(profile.get("title") or context.get("title") or "")[:240],
        "study_family_id": profile.get("study_family_id"),
...
def _cluster_synthesis_profile_projection(
    profile: Mapping[str, Any],
    cluster: Mapping[str, Any],
    source_note: Mapping[str, Any] | None = None,
    *,
    ...
    selected = [
        {
            ...
            "source_id": str(anchor.get("source_id") or profile.get("source_id") or ""),
            "claim": str(anchor.get("text") or anchor.get("claim") or "")[:600],
            "locator": str(anchor.get("locator") or "")[:160],
            "planning_roles": list(anchor.get("planning_roles", []) or []),
            "salience_priority": _nonnegative_int(anchor.get("salience_priority")),
            "support_envelope": {
                key: value
                for key, value in _as_mapping(anchor.get("support_envelope")).items()
                if key
                in {
                    "argument_role",
                    "coverage",
    ...
    if source_note is not None:
        _frontmatter, body = source_note_semantic_components(
            str(source_note.get("body") or "")
        )
        compact.update(
            {
                "title": str(source_note.get("title") or compact.get("title") or ""),
                "authors": list(source_note.get("creators", []) or []),
                "year": str(source_note.get("date") or ""),
                "source_scope": str(source_note.get("source_scope") or ""),
                "atomic_note_markdown": body.strip(),
            }
        )
    return compact
...
def _cluster_plan_call_settings(
    ...
    *,
    ...
    usable_input_tokens = max(
        0,
        int(context_tokens * 0.75) - desired_output - prompt_reserve,
    )
...
def _global_plan_proposals(
    ...
    for raw_cluster in response.get("clusters", []) or []:
        ...
        for member in members:
            ...
            if not anchor_ids:
                continue
            # Legacy admission code still expects a core/context role. v0.14
            # treats every planner-retained member as evidentiary; role labels
            # are descriptive and cannot suppress a source's findings.
            roles[source_id] = "core"
        ...
        organizing_mode = str(
            raw_cluster.get("organizing_mode") or "question"
        ).strip()
        organizing_problem = str(
            raw_cluster.get("organizing_problem")
            or raw_cluster.get("bounded_object")
            or raw_cluster.get("shared_question")
            or ""
        ).strip()
        question = str(
            raw_cluster.get("guiding_question")
            or raw_cluster.get("shared_question")
            or ""
        ).strip()
        rationale = str(
        ...
        proposals.append(
            {
                ...
                "central_tension": str(
                    raw_cluster.get("central_tension") or ""
                ).strip(),
                "shared_question": question or organizing_problem,
                "bounded_object": str(
                    organizing_problem or semantic_identity
                ),
...
def _reasoner_context_char_budget(reasoner: Any, request: Any) -> int:
    ...
    policy = request_values.get("literature_policy")
    context_tokens = int(getattr(reasoner, "context_window_tokens", 0) or 128_000)
    ...
    usable_tokens = max(
        0,
        int(context_tokens * input_fraction)
        - output_reserve
        - int(getattr(reasoner, "prompt_reserve_tokens", 0) or 0),
    )
    # Match the reader's conservative UTF-8 token estimate and leave prompt
    # serialization outside the measured profiles/context packet.
    return max(8_000, usable_tokens * 3)


def _reasoner_packet_chars(
    profiles: Sequence[Mapping[str, Any]], context: Mapping[str, Any]
) -> int:
    """Measure the complete provider-visible cluster packet."""

    return len(
        json.dumps(
            {"profiles": list(profiles), "context": dict(context)},
            sort_keys=True,
            ensure_ascii=False,
            default=str,
        )
...
def _pack_source_ids_by_size(
    ...
    *,
    ...
    for source_id in source_ids:
        ...
        size = len(
            json.dumps(
                _coverage_profile_projection(profile),
                sort_keys=True,
                ensure_ascii=False,
                default=str,
            )
        ...
        if current and (
            len(current) >= max_sources or current_chars + size > max_chars
        ):
            chunks.append(current)
            current = []
            current_chars = 0
        current.append(str(source_id))
        current_chars += size
    if current:
        chunks.append(current)
    if len(chunks) > 1 and len(chunks[-1]) == 1:
        ...
        candidate_size = sum(
            len(
                json.dumps(
                    _coverage_profile_projection(profile_by_source.get(value, {})),
                    sort_keys=True,
                    ensure_ascii=False,
                    default=str,
                )
        ...
        if len(candidate) <= max_sources and candidate_size <= max_chars:
            chunks[-2] = candidate
            chunks.pop()
        elif len(prior) > 2:
            rebalanced_tail = [prior[-1], *tail]
            rebalanced_size = sum(
                len(
                    json.dumps(
                        _coverage_profile_projection(
                            profile_by_source.get(value, {})
                        ),
                        sort_keys=True,
                        ensure_ascii=False,
                        default=str,
                ...
                for value in rebalanced_tail
            )
            if rebalanced_size <= max_chars:
                chunks[-2] = prior[:-1]
                chunks[-1] = rebalanced_tail
    return chunks
...
def _coverage_audit_plan(
    ...
    *,
    ...
    max_chars = _reasoner_context_char_budget(reasoner, request)
    ...
    components = [
        ...
        if len(component.get("source_ids", [])) >= 2
    ]
    if not components:
        components = [
            {
                "focus_source_ids": sorted(focus_source_ids),
                "source_ids": sorted(focus_source_ids),
            }
        ]
    plan = []
    for component in components:
        source_chunks = _pack_source_ids_by_size(
            component["source_ids"],
            profile_by_source,
            max_chars=max_chars,
        )
        for source_ids in source_chunks:
            focus_ids = sorted(set(component["focus_source_ids"]) & set(source_ids))
            if len(source_ids) < 2 or not focus_ids:
                continue
            packet = {
    ...
    for response in responses:
        ...
        for raw in response.get("clusters", []) or []:
            ...
            if key not in merged:
                merged[key] = proposal
                continue
            prior_id = str(merged[key].get("proposal_id") or "")
            proposal_id = str(proposal.get("proposal_id") or "")
            if prior_id and proposal_id and prior_id == proposal_id:
                # A later collection audit may deliberately narrow a complete
                # proposal after seeing current admission failures. Treat the
                # same proposal ID as a replacement; union remains appropriate
                # only for distinct whole-profile shards of one conversation.
                merged[key] = proposal
            else:
                merged[key] = merge_rows(merged[key], proposal)
    return list(merged.values())


def _cluster_relationship_context(
    ...
    for relation in relationships:
        ...
        trusted_structural = (
            ...
            or "zotero" in provenance
            or relation_type in {"cites", "cited_by", "zotero_related"}
        )
        if not trusted_structural and str(
            relation.get("verification_status") or ""
        ) not in {
            "confirmed",
            "corrected",
            "final",
...
def _cluster_proposal_partitions(
    profiles: Sequence[Mapping[str, Any]],
    catalogue_shards: Sequence[Mapping[str, Any]],
    ...
    *,
    ...
    """Build bounded shard and verified-bridge packets without semantic inference."""
    ...
    source_literatures: dict[str, set[str]] = defaultdict(set)
    ...
    def add_partition(
        ...
        identity: str,
        source_ids: Sequence[str],
        routing_cards: Sequence[Mapping[str, Any]] = (),
    ) -> None:
        values = sorted({str(value) for value in source_ids if str(value) in analytical})
        chunks = _pack_source_ids_by_size(
            values, profile_by_source, max_chars=max_chars
        )
        for index, chunk in enumerate(chunks, start=1):
            if len(chunk) < 2:
                continue
            payload = {
                "partition_id": f"{identity}-{index:02d}",
                "route": route,
                "source_ids": chunk,
                "routing_cards": [dict(row) for row in routing_cards],
            }
            payload["revision_hash"] = _stable_hash(payload)
            partitions.append(payload)
            covered.update(chunk)

    for shard in catalogue_shards:
        if not isinstance(shard, Mapping):
            continue
        shard_id = str(shard.get("shard_id") or "")
        literature_id = str(shard.get("literature_id") or "")
        source_ids = [
            str(value)
            for value in shard.get("source_ids", []) or []
            if str(value) in analytical
        ]
        for source_id in source_ids:
            if literature_id:
                source_literatures[source_id].add(literature_id)
        routing_card = (
            [dict(shard["routing_card"])]
            if isinstance(shard.get("routing_card"), Mapping)
            else []
        )
        add_partition("catalogue_shard", shard_id or "shard", source_ids, routing_card)

    uncovered = sorted(analytical - covered)
    add_partition("fallback_chunk", "unrouted", uncovered)
    ...
    for relation in accepted_relationships:
        ...
        right = str(relation.get("target_source_id") or "")
        if (
            left not in analytical
            or right not in analytical
            or not source_literatures.get(left)
            or not source_literatures.get(right)
            or source_literatures[left] & source_literatures[right]
        ):
            continue
        adjacency[left].add(right)
        adjacency[right].add(left)
    ...
    for source_id in sorted(adjacency):
        ...
        add_partition(
            "verified_bridge_component",
            "bridge-" + _stable_hash(sorted(component))[:12],
            sorted(component),
        )
    return partitions


def _cluster_context_rows(
...
def build_literature_report(
    ...
    *,
    ...
    cluster_plan_state: dict[str, Any] = {}
    if uses_global_cluster_plan and len(analytical_families) >= 2:
        collection_memberships = _as_mapping(
            (source_set or {}).get("collection_memberships")
        )
        planning_cards = [
            {
                **_cluster_planning_card(row),
                "collections": list(
                    collection_memberships.get(
                        str(row.get("source_id") or ""), []
                    )
                    or []
                ),
        ...
        collection_identity["collection_views"] = list(
        ...
        source_card_hashes = {
            str(card.get("source_id") or ""): _stable_hash(card)
            for card in planning_cards
            if card.get("source_id")
        }
        source_state_hash = _stable_hash(source_card_hashes)
        planning_identity = {
            "prompt_version": CLUSTER_PLAN_PROMPT_VERSION,
            "algorithm_version": LITERATURE_ALGORITHM_VERSION,
            "provider": str(getattr(reasoner, "name", "") or ""),
            "model": str(getattr(reasoner, "model", "") or ""),
            "question": str(question or ""),
            "policy": _semantic_literature_policy(
                _as_mapping(_as_mapping(request).get("literature_policy"))
            ),
        }
        planning_context_hash = _stable_hash(
            {
                "accepted_relationships": accepted_plan_relationships,
                "literature_positions": literature_positions,
                "collection_identity": collection_identity,
                "rejected_pair_memory": rejected_pair_memory,
            }
        )
        prior_plan_state = _as_mapping(
            (previous_registry or {}).get("cluster_plan_state")
        )
        prior_plan_response = _as_mapping(prior_plan_state.get("plan_response"))
        reuse_prior_plan = (
            prior_plan_state.get("status") == "complete"
            and prior_plan_state.get("source_card_hashes") == source_card_hashes
            and str(prior_plan_state.get("source_state_hash") or "")
            == source_state_hash
            and str(prior_plan_state.get("planning_context_hash") or "")
            == planning_context_hash
            and prior_plan_state.get("planning_identity") == planning_identity
            and isinstance(prior_plan_response.get("clusters", []), list)
        )
        ...
        plan_context = {
            ...
            "collection_identity": collection_identity,
            "rejected_pair_memory": rejected_pair_memory,
            "cluster_plan_mode": "global",
            "cluster_plan_settings": plan_settings,
        }
        prior_card_hashes = _as_mapping(
            prior_plan_state.get("source_card_hashes")
        )
        if (
            prior_card_hashes
            and prior_card_hashes != source_card_hashes
            and prior_plan_state.get("planning_identity") == planning_identity
        ):
            changed_source_ids = sorted(
                source_id
                for source_id in set(prior_card_hashes) | set(source_card_hashes)
                if prior_card_hashes.get(source_id)
            ...
            prior_family_cards = [
                {
                    ...
                    "organizing_mode": str(
                        cluster.get("organizing_mode") or ""
                    ),
                    "organizing_problem": str(
                ...
                for cluster in (previous_registry or {}).get("clusters", []) or []
                if isinstance(cluster, Mapping) and cluster.get("cluster_id")
            ]
            plan_context.update(
                incremental_source_delta={
                    "changed_source_ids": changed_source_ids,
                    "prior_source_state_hash": str(
                        prior_plan_state.get("source_state_hash") or ""
                    ),
                    "source_state_hash": source_state_hash,
                },
                prior_active_cluster_family_cards=prior_family_cards,
                prior_family_state_hash=_stable_hash(prior_family_cards),
            )
        ...
        if reuse_prior_plan:
            plan_response = dict(prior_plan_response)
        elif (
            ...
            pending_packets = _cluster_plan_packets(
                ...
                preserve_shard_boundaries=True,
            )
                ...
                for future in as_completed(future_map):
                    ...
                    try:
                        ...
                        source_ids = sorted(
                            ...
                            for card in packet.get("cards", []) or []
                            if isinstance(card, Mapping)
                            and card.get("source_id")
                        )
                        local_by_index[index] = {
                            "clusters": [],
                            "neighbor_relationships": [],
                            "unclustered_sources": [
                                {
                                    "source_id": source_id,
                                    "reason": "cluster_plan_packet_failed",
                                }
                                for source_id in source_ids
                            ],
                            "parked_clusters": [
            ...
            local_plan_responses = [
                local_by_index[index]
                for index in range(len(prepared_packets))
            ]
    ...
    rendered_partitions: list[dict[str, Any]] = []
    while partition_queue:
        partition = dict(partition_queue.pop(0))
        partition_source_ids = set(partition["source_ids"])
        partition_profiles = [
            _coverage_profile_projection(row)
            for row in normalized
        ...
        partition_context = {
            "propositions": _cluster_context_rows(
                propositions, partition_source_ids
            ),
            "relations": compact_verified_relations,
            "accepted_relationships": compact_verified_relations,
            "relationship_cluster_candidates": [
                dict(row)
                for row in relationship_candidates
                if isinstance(row, Mapping)
                and row.get("target_kind") == "cluster"
                and str(row.get("source_id") or "") in partition_source_ids
            ],
            "coverage_component_source_ids": sorted(partition_source_ids),
            "coverage_focus_source_ids": sorted(partition_source_ids),
            "coverage_audit_mode": str(partition["route"]),
            "coverage_component_signature": str(partition["revision_hash"]),
            "partition_source_ids": sorted(partition_source_ids),
            "partition_revision": str(partition["revision_hash"]),
            "routing_cards": list(partition["routing_cards"]),
        }
        if (
            _reasoner_packet_chars(partition_profiles, partition_context)
            > context_char_budget
        ):
            source_ids = sorted(partition_source_ids)
            if len(source_ids) < 4:
                raise ValueError(
                    "cluster proposal packet exceeds provider context budget "
                    f"for partition {partition['partition_id']}"
                )
            midpoint = len(source_ids) // 2
            children = (source_ids[:midpoint], source_ids[midpoint:])
            partition_queue[0:0] = [
                {
                    **partition,
                    ...
                    "revision_hash": _stable_hash(
                        {
                            "parent_revision": partition["revision_hash"],
                            "source_ids": child_ids,
                        }
                    ),
                }
                for index, child_ids in enumerate(children, start=1)
            ]
            continue
        rendered_partitions.append(partition)
        proposal_responses.append(
            _reasoner_stage(
                reasoner,
                reasoner_call,
                stage="cluster_proposal",
                key=str(partition["partition_id"]),
    ...
    if (
        ...
        reconciliation_context = {
            "coverage_audit_mode": "reconciliation",
            "prior_proposals": combined_proposals,
            "prior_proposal_identities": [
                str(row.get("semantic_identity") or row.get("label") or "")
                for row in combined_proposals
            ],
            "relations": reconciliation_relationships,
            "accepted_relationships": reconciliation_relationships,
            "routing_cards": [
                dict(row)
                for partition in partitions
                for row in partition.get("routing_cards", []) or []
                if isinstance(row, Mapping)
            ],
            "partition_revision": _stable_hash(
                [str(partition["revision_hash"]) for partition in partitions]
            ),
        }
        if _reasoner_packet_chars([], reconciliation_context) > context_char_budget:
            raise ValueError(
                "cluster reconciliation packet exceeds provider context budget"
            )
        reconciliation_response = _reasoner_stage(
            reasoner,
            reasoner_call,
            stage="cluster_reconciliation",
            key="families-" + _stable_hash(combined_proposals)[:16],
            method_name="propose_clusters",
            profiles=[],
            request=request,
            context=reconciliation_context,
        )
        proposal_responses.append(reconciliation_response)
        combined_proposals = _cluster_proposals_from_responses(
            proposal_responses
        )
    ...
    if uses_global_cluster_plan:
        ...
        for cluster in clustered.get("clusters", []) or []:
            proposal = proposal_by_id.get(
                str(cluster.get("proposal_id") or "")
            )
            if proposal is None:
                continue
            for field in (
                ...
                cluster[field] = str(proposal.get(field) or "")
    failed_plan_source_ids = {
        str(source_id)
        for row in global_plan_parked
        for source_id in row.get("failed_source_ids", []) or []
        if str(source_id)
    }
    if failed_plan_source_ids:
        ...
        for prior in (previous_registry or {}).get("clusters", []) or []:
            if not isinstance(prior, Mapping):
                continue
            cluster_id = str(prior.get("cluster_id") or "")
            if (
                not cluster_id
                or cluster_id in current_cluster_ids
                or not (
                    failed_plan_source_ids
                    & {
                        str(value)
                        for value in prior.get("source_ids", []) or []
                        if str(value)
                    }
                )
            ):
                continue
            clustered["clusters"].append(
                {
                    **dict(prior),
    ...
    while coverage_queue:
        ...
        audit_source_id_set = set(audit_source_ids)
        audit_profiles = [
            _coverage_profile_projection(row)
            for row in normalized
        ...
        audit_context = {
            "propositions": _cluster_context_rows(
                propositions, audit_source_id_set
            ),
            "relations": [
                *_cluster_context_rows(relations, audit_source_id_set),
                *compact_verified_relations,
            ],
            "topic_neighborhoods": _cluster_context_rows(
                topic_neighborhoods, audit_source_id_set
            ),
            ...
            "coverage_audit_mode": str(
                audit.get("mode") or "semantic_component"
            ),
            ...
            "current_clusters": _cluster_context_rows(
                clustered.get("clusters", []) or [], audit_source_id_set
            ),
            "current_unclustered_sources": _cluster_context_rows(
            ...
            "prior_proposals": _cluster_context_rows(
                combined_proposals, audit_source_id_set
            ),
            "study_lineages": _cluster_context_rows(
                independence["study_lineages"], audit_source_id_set
            ),
            "evidence_base_groups": _cluster_context_rows(
                independence["evidence_base_groups"], audit_source_id_set
            ),
            "independence_assessments": _cluster_context_rows(
                independence["independence_assessments"],
                audit_source_id_set,
            ),
            "accepted_relationships": compact_verified_relations,
            "relationship_cluster_candidates": [
                ...
                if isinstance(row, Mapping)
                and row.get("target_kind") == "cluster"
                and str(row.get("source_id") or "") in audit_source_id_set
            ],
            "partition_source_ids": sorted(audit_source_id_set),
            "partition_revision": audit_key,
        }
        if _reasoner_packet_chars(audit_profiles, audit_context) > context_char_budget:
            source_ids = sorted(audit_source_id_set)
            if len(source_ids) < 4:
                raise ValueError(
                    "cluster coverage packet exceeds provider context budget "
                    f"for audit {audit_key}"
                )
            midpoint = len(source_ids) // 2
            coverage_queue[0:0] = [
                {
                    **audit,
                    "key": f"{audit_key}--{index}",
                    "source_ids": child_ids,
                    "focus_source_ids": sorted(
                        set(focus_source_ids) & set(child_ids)
                    ),
                }
                for index, child_ids in enumerate(
                    (source_ids[:midpoint], source_ids[midpoint:]),
                    start=1,
                )
            ]
            continue
    ...
    registry["derived_from_source_state_hash"] = _stable_hash(
        sorted(
            (
                str(row.get("source_id") or ""),
                str(
                    row.get("source_analysis_bundle_dependency_fingerprint")
                    or row.get("dependency_hash")
                    or row.get("note_hash")
                    or ""
                ),
            )
            for row in normalized
    ...
    registry["planning_identity"] = {
        "prompt_version": CLUSTER_PLAN_PROMPT_VERSION,
        "algorithm_version": LITERATURE_ALGORITHM_VERSION,
        "provider": str(getattr(reasoner, "name", "") or ""),
        "model": str(getattr(reasoner, "model", "") or ""),
    }
    if cluster_plan_state:
        registry["cluster_plan_state"] = cluster_plan_state
    registry["pending_revisions"] = [
        dict(row)
        for row in (previous_registry or {}).get("pending_revisions", []) or []
        if isinstance(row, Mapping)
    ]
    ...
    writer_membership_changed = False
    ...
    def apply_writer_membership(
        ...
        retained = {
            ...
            if str(source_id) in set(cluster.get("source_ids", []) or [])
        }
        if len(retained) < 2:
            return
        ...
        cluster["note_ids"] = sorted(
            str(normalized_by_source[source_id].get("note_id") or "")
            for source_id in ordered
            if normalized_by_source[source_id].get("note_id")
        )
        families = sorted(
            {
                str(
        ...
        cluster["proposal_supporting_evidence"] = [
            dict(row)
            for row in cluster.get("proposal_supporting_evidence", []) or []
            if isinstance(row, Mapping)
            and str(row.get("source_id") or "") in retained
        ]
        cluster["revision_hash"] = _stable_hash(
            {
                ...
                "source_ids": ordered,
                "cluster_writer_contract": "streamlined-full-note-v1",
            }
        )
    ...
    def cluster_job_inputs(
        cluster: Mapping[str, Any],
    ) -> tuple[list[dict[str, Any]], dict[str, Any]]:
        cluster_id = str(cluster.get("cluster_id") or "")
        member_ids = set(cluster.get("source_ids", []) or [])
        member_profiles = [
            row
            for row in normalized
            if str(row.get("source_id") or "") in member_ids
        ]
        synthesis_profiles = [
        ...
        planned_neighbor_ids = {
            ...
            if isinstance(row, Mapping) and row.get("target_cluster_id")
        }
        cluster_card = {
            "cluster_id": cluster_id,
            "title": str(cluster.get("label") or ""),
            "label": str(cluster.get("label") or ""),
            "semantic_identity": str(cluster.get("semantic_identity") or ""),
            "organizing_mode": str(
            ...
            "shared_question": str(
                cluster.get("shared_question")
                or cluster.get("organizing_problem")
                or cluster.get("bounded_object")
                or ""
            ),
            "bounded_object": str(
            ...
            "central_tension": str(cluster.get("central_tension") or ""),
            "coherence_rationale": str(
                cluster.get("coherence_rationale") or ""
            ),
            "source_ids": sorted(member_ids),
            "proposition_ids": list(cluster.get("proposition_ids", []) or []),
            "propositions": [
                dict(row)
                for row in cluster.get("propositions", []) or []
                if isinstance(row, Mapping)
        ...
        return synthesis_profiles, {
            ...
            "neighbor_clusters": [
                row
                for row in cluster_context_catalog
                if str(row.get("cluster_id") or "") in planned_neighbor_ids
            ],
            "planned_neighbor_relationships": planned_neighbors,
        }

    concurrent_results: dict[str, Mapping[str, Any] | BaseException] = {}
    if uses_global_cluster_plan and synthesis_order:
        runnable = []
        ...
        def run_cluster_job(
            ...
            try:
                profiles_for_job, context_for_job = cluster_job_inputs(cluster)
                return _reasoner_stage(
                    reasoner,
                    reasoner_call,
                    stage="cluster_synthesis",
                    key=str(cluster.get("cluster_id") or ""),
                    method_name="synthesize_cluster",
                    profiles=profiles_for_job,
                    request=request,
        ...
        _notify_stage(
            ...
            "cluster_synthesis",
            active_cluster="",
            cluster_peak_concurrency=peak_jobs,
            cluster_stage_wall_seconds=round(
                time.monotonic() - synthesis_started, 3
            ),
        )
    ...
    for cluster in synthesis_order:
        ...
        try:
            if uses_global_cluster_plan:
                result_or_error = concurrent_results.get(cluster_id)
                if isinstance(result_or_error, BaseException):
                    raise result_or_error
                synthesis_response = _as_mapping(result_or_error)
            else:
        ...
        if (
            ...
            pending_source_ids = sorted(
                set(attempted_cluster.get("source_ids", []) or [])
                ^ set(prior_cluster.get("source_ids", []) or [])
            )
            pending_revision_hash = str(
                attempted_cluster.get("revision_hash") or ""
            )
            pending_record = {
                "cluster_id": cluster_id,
                ...
                "cluster": attempted_cluster,
                "synthesis": dict(validated_synthesis),
            }
            registry["pending_revisions"] = [
                row
                for row in registry["pending_revisions"]
                if str(row.get("cluster_id") or "") != cluster_id
            ]
            registry["pending_revisions"].append(pending_record)
            cluster.clear()
            cluster.update(
                prior_cluster,
                refresh_pending=True,
    ...
    rejected_cluster_ids = {
        cluster_id
        for cluster_id, synthesis in cluster_syntheses.items()
        if synthesis.get("status") == "rejected"
    }
    if rejected_cluster_ids:
        registry["clusters"] = [
            cluster
            for cluster in registry["clusters"]
            if str(cluster.get("cluster_id") or "") not in rejected_cluster_ids
        ]
        clustered["clusters"] = list(registry["clusters"])
        reverted_refresh = True
        for cluster_id in rejected_cluster_ids:
            registry["ledger"].append(
                {
                    "event": "cluster_rejected_by_writer",
                    "cluster_id": cluster_id,
                }
            )
    ...
    # v0.15 trusts the full-note writer's same-call statistical review. The
    # legacy comparison audit remains only for custom pre-v0.14 reasoners.
    quantitative_comparisons = (
        []
        if uses_global_cluster_plan
    ...
    # the provider-free legacy audit for custom reasoner compatibility.
    generated_candidates = (
        []
        if uses_global_cluster_plan
        else generate_gap_candidates(
            normalized,
            registry["clusters"],
            debates,
            matrices,
    ...
    manifest = {
        ...
        "refresh_pending_cluster_count": len(
            registry.get("pending_revisions", []) or []
        ),
        ...
        packet.update(
            {
                ...
                "partial_reason": (
                    ...
                    if failed_plan_source_ids
                        ...
                        "incomplete_cluster_synthesis:"
                        if incomplete_synthesis_ids
                        else "cluster_refresh_pending:"
                    )
                )
                + ",".join(
                    sorted(failed_plan_source_ids)
                    if failed_plan_source_ids
                    else partial_cluster_ids
                ),
                "partial_cluster_ids": partial_cluster_ids,
                "planning_failure_source_ids": sorted(
                    failed_plan_source_ids
                ),
            }
        )
...
def _prune_stale_generated_markdown(
    ...
    for path in directory.glob("*.md"):
        if (
            ...
            and "<!-- auto-zettelkasten:cluster:start -->"
...
_CLUSTER_MANAGED_START = "<!-- auto-zettelkasten:cluster:start -->"
_CLUSTER_MANAGED_END = "<!-- auto-zettelkasten:cluster:end -->"


def _managed_cluster_projection(markdown: str) -> str:
    end = markdown.find("\n---\n", 4) if markdown.startswith("---\n") else -1
    prefix = markdown[: end + 5] if end >= 0 else ""
    body = markdown[end + 5 :] if end >= 0 else markdown
    return (
        f"{prefix}{_CLUSTER_MANAGED_START}\n"
        f"{body.rstrip()}\n{_CLUSTER_MANAGED_END}\n"
    )


def _park_cluster_projection(
    workspace: Path, path: Path, cluster_id: str, reason: str
) -> None:
...
def _resolve_cluster_projection(
    workspace: Path, *, cluster_id: str
) -> None:
    ledger_path = workspace / "11_state" / "projection_failures.yml"
    existing = read_yaml(ledger_path, {}) or {}
    active = [
    ...
    resolved_now = [
        row
        for row in active
        if row.get("artifact_kind") == "cluster_note"
        and row.get("artifact_id") == cluster_id
    ]
    if not resolved_now:
        return
    resolved = {
    ...
    write_yaml(
        ...
        {
            "projection_failure_schema_version": "1",
            "failures": [row for row in active if row not in resolved_now],
            "resolved_failures": [resolved[key] for key in sorted(resolved)],
        },
    )


def _write_managed_cluster_projection(
    workspace: Path,
    path: Path,
    markdown: str,
    *,
    ...
    generated = _managed_cluster_projection(markdown)
    if not path.is_file():
        atomic_write_text(path, generated)
        _resolve_cluster_projection(workspace, cluster_id=cluster_id)
        return True
    text = path.read_text(encoding="utf-8")
    start_count = text.count(_CLUSTER_MANAGED_START)
    end_count = text.count(_CLUSTER_MANAGED_END)
    refresh_start_count = text.count(_REFRESH_PENDING_START)
    refresh_end_count = text.count(_REFRESH_PENDING_END)
    if (
        (start_count, end_count) not in {(0, 0), (1, 1)}
        or (refresh_start_count, refresh_end_count) not in {(0, 0), (1, 1)}
    ):
        _park_cluster_projection(
            workspace, path, cluster_id, "ambiguous_managed_cluster_block"
        )
        return False
    generated_end = generated.find("\n---\n", 4)
    managed = (
        generated[generated_end + 5 :]
        if generated_end >= 0
        else generated
    )
    refresh_pattern = (
        rf"{re.escape(_REFRESH_PENDING_START)}.*?"
        rf"{re.escape(_REFRESH_PENDING_END)}\n*"
    )
    text_without_refresh = re.sub(
        refresh_pattern,
        "",
        text,
        flags=re.DOTALL,
    ...
    if start_count == 1 and existing is None:
        _park_cluster_projection(
            workspace, path, cluster_id, "ambiguous_managed_cluster_block"
        )
        return False
    desired = (
        f"{text_without_refresh[:existing.start()]}{managed}"
        f"{text_without_refresh[existing.end():]}"
        if existing
        else (
            f"{text_without_refresh}"
            f"{'' if text_without_refresh.endswith(chr(10)) else chr(10)}\n"
            f"{managed}"
        )
    )
    if desired == text:
...
def _mark_cluster_refresh_pending(
    ...
    start_count = text.count(_REFRESH_PENDING_START)
    end_count = text.count(_REFRESH_PENDING_END)
    ...
    if existing:
        desired = f"{text[:existing.start()]}{warning}{text[existing.end():]}"
    elif text.startswith("---\n") and (end := text.find("\n---\n", 4)) >= 0:
        desired = f"{text[:end + 5]}\n\n{warning}\n{text[end + 5:]}"
    else:
        desired = f"{warning}\n\n{text}"
    if desired != text:
        atomic_write_text(path, desired)
...
def _streamlined_cluster_markdown(
    cluster: Mapping[str, Any],
    synthesis: Mapping[str, Any],
    *,
    profile_by_source: Mapping[str, Mapping[str, Any]],
    ...
    title = str(
        synthesis.get("title") or cluster_display_title(cluster)
    ).strip()
    retained_ids = [
        str(value)
    ...
    frontmatter = {
        ...
        "title": title,
        "cluster_id": str(cluster.get("cluster_id") or ""),
        "organizing_mode": str(synthesis.get("organizing_mode") or ""),
        "tags": _cluster_obsidian_tags(cluster),
        "sources": [_obsidian_note_link(row) for row in members],
        "related_clusters": list(dict.fromkeys(related_links)),
    }
    sections = [f"# {title}"]
    organizing_problem = _human_projection_text(
        synthesis.get("organizing_problem") or ""
    )
    if organizing_problem:
        sections.append(f"## Organizing problem\n\n{organizing_problem}")
    bottom_line = _human_projection_text(synthesis.get("bottom_line") or "")
    if bottom_line:
        sections.append(f"## Bottom line\n\n{bottom_line}")

    line_sections: list[str] = []
    for line in synthesis.get("lines_of_inquiry", []) or []:
        if not isinstance(line, Mapping):
            continue
        heading = _human_projection_text(line.get("title") or "Line of inquiry")
        prose = _human_projection_text(line.get("synthesis") or "")
        rows = [f"### {heading}"]
        if prose:
            rows.append(prose)
        for finding in line.get("study_findings", []) or []:
            ...
            text = _human_projection_text(finding.get("finding") or "")
            method_scope = _human_projection_text(
                finding.get("method_scope") or ""
            )
            technical_result = _human_technical_result(
                finding.get("technical_result") or ""
            )
            plain_english = _human_projection_text(
                finding.get("plain_english_meaning") or ""
            )
            locators = [
                _human_locator_text(reference.get("locator") or "")
                for reference in finding.get("evidence", []) or []
                if isinstance(reference, Mapping)
            ]
            suffix = "; ".join(value for value in locators if value)
            detail = text
            if method_scope:
                detail += f" ({method_scope})"
            if suffix:
                detail += f" — {suffix}"
            rows.append(f"- {_obsidian_note_link(source)}: {detail}")
            if technical_result and technical_result.casefold() not in {
                "not reported",
                "not_reported",
            }:
                rows.append(f"  - Technical result: {technical_result}")
            if (
                plain_english
                and plain_english.casefold() not in text.casefold()
            ):
                rows.append(f"  - In plain English: {plain_english}")
        line_sections.append("\n\n".join(rows))
    if line_sections:
        sections.append("## Main lines of inquiry\n\n" + "\n\n".join(line_sections))

    differences = []
    for row in synthesis.get("differences", []) or []:
    ...
    if related_links:
        sections.append(
            "## Related clusters\n\n"
            + "\n".join(f"- {value}" for value in dict.fromkeys(related_links))
        )
    sections.append(
        "## Members\n\n"
        + "\n".join(f"- {_obsidian_note_link(row)}" for row in members)
    )
    return _replace_source_keys_in_markdown_body(
        _markdown_with_frontmatter(frontmatter, "\n\n".join(sections)),
        list(profile_by_source.values()),
...
def _cluster_markdown(
    ...
    *,
    ...
    representative_by_source = {
        str(row.get("source_id") or ""): row
        for row in representative_sources
        if isinstance(row, Mapping) and row.get("source_id")
    }
    member_sources = [
        profile_by_source.get(source_id)
        or representative_by_source.get(source_id)
    ...
    source_links = [_obsidian_note_link(row) for row in member_sources]
    ...
    def strict_claim_explanation(adjudication: Mapping[str, Any]) -> str:
        ...
        if (
            kind == "contradiction"
            and str(adjudication.get("decision") or "not_established")
            != "established"
            and not any(
                token in detail.casefold()
                for detail in details
                for token in ("opposite", "opposing")
            )
        ):
            details.append(
                "No two comparable sources reach genuinely opposite conclusions on the same claim."
    ...
    if thread_rows:
        ...
        if thread_titles:
            readable_thread_titles = [
                title[:1].lower() + title[1:]
                if title[:1].isupper()
                and not re.match(r"^[A-Z]{2,}(?:\b|\s)", title)
                else title
                for title in thread_titles
            ]
            fit_lines.append(
                "The literature is organized around "
                + ", ".join(readable_thread_titles[:-1])
                + (
                    f", and {readable_thread_titles[-1]}"
                    if len(readable_thread_titles) > 1
                    else readable_thread_titles[0]
                )
                + "."
            )
    ...
    for row in ranked_contributions:
        ...
        if not source_id:
            continue
        limit = 3 if role_by_source.get(source_id) == "core" else 1
        selected = selected_by_source[source_id]
        if len(selected) >= limit:
            continue
        row_terms = _tokens(row.get("finding") or "") - _BROAD_FIELD_TERMS
        if row_terms and any(
            len(row_terms & (_tokens(prior.get("finding") or "") - _BROAD_FIELD_TERMS))
            / max(
                1,
                min(
                    len(row_terms),
                    len(_tokens(prior.get("finding") or "") - _BROAD_FIELD_TERMS),
                ),
            )
            >= 0.7
    ...
    core_contributions = [
        ...
        if role_by_source.get(source_id) == "core"
        for row in selected_by_source.get(source_id, [])
    ]
    ...
    def contribution_inferential_limit(
        contribution: Mapping[str, Any],
    ) -> str:
        for reference in contribution.get("evidence", []) or []:
            if not isinstance(reference, Mapping):
                continue
            for value in reference.get("restrictions", []) or []:
                text = _human_projection_text(value)
                if text and re.search(
                    r"\b(?:caus|cannot rule out|counterfactual|confound|selection|"
                    r"observational|single[- ]case|not test)\w*\b",
                    text,
                    flags=re.I,
                ):
                    return text
    ...
    for thread in thread_rows:
        ...
        for contribution in contributions_by_thread.get(thread_id, []):
            ...
            inferential_limit = contribution_inferential_limit(contribution)
            if inferential_limit:
                findings.append(f"  - Inferential limit: {inferential_limit}")
    unassigned = [
        row
        for row in contributions
...
def _gap_markdown(
    ...
    *,
    ...
    if missing_cell.get("description"):
        missing_cell_text = _safe_gap_resolution_text(
            missing_cell.get("description"), fallback=statement
        )
        if missing_cell_text:
            lineage_lines.append(
                f"- Missing evidence-matrix cell: {missing_cell_text}"
            )
    if lineage_lines:
    ...
    resolution_path_rejected = any(
        not bool(check.get("passed"))
        and (
            "resolution path" in str(check.get("requirement") or "").casefold()
            or "type-sensitive path"
            in str(check.get("requirement") or "").casefold()
            or "resolution path" in str(check.get("explanation") or "").casefold()
        )
        for check in strict_checks
    )
    if resolution and not resolution_path_rejected:
        path_type = str(resolution.get("path_type") or "")
        requirements = _as_mapping(resolution.get("requirements"))
        required_fields = _RESOLUTION_PATH_REQUIREMENTS.get(path_type, ())
        missing_fields = [
    ...
    contribution = (
        ""
        if resolution_path_rejected
        else _safe_gap_significance_text(gap.get("contribution") or "")
    )
    if why_matters:
...
def _map_unclustered_reason_label(value: Any) -> str:
    ...
    labels = {
        "currently_unclustered": "Currently unclustered",
...
def _load_map_cluster_registry(workspace: Path, map_id: str) -> Mapping[str, Any]:
    ...
    for legacy in (
        ...
        if isinstance(payload, Mapping):
            result = dict(payload)
            syntheses = read_yaml(legacy.parent / "cluster_syntheses.yml", {}) or {}
            result["cluster_syntheses"] = dict(
                syntheses.get("syntheses", {}) or {}
            )
            return result
...
def _preserve_last_valid_clusters_on_refresh_failure(
    workspace: Path,
    map_id: str,
    reason: str,
    ...
    ledger_without_ids = [
        dict(row)
        for row in registry.get("ledger", []) or []
        if isinstance(row, Mapping) and not row.get("event_id")
    ]
    for cluster in clusters:
        cluster_id = str(cluster["cluster_id"])
        ...
        cluster.update(
            refresh_pending=True,
            last_good_revision_hash=revision_hash,
            pending_revision_hash="",
            refresh_pending_source_ids=[],
        )
        pending_by_id[cluster_id] = {
            "cluster_id": cluster_id,
            "pending_revision_hash": "",
            "last_good_revision_hash": revision_hash,
            "refresh_pending_source_ids": [],
        ...
        event_id = _stable_hash(event)
        ledger_by_id[event_id] = {"event_id": event_id, **event}
    ...
    _write_projection_yaml(
        registry_path,
        {
            **dict(registry),
            ...
            "clusters": clusters,
            "pending_revisions": [
                pending_by_id[key] for key in sorted(pending_by_id)
            ],
            "ledger": [
                *ledger_without_ids,
                *(ledger_by_id[key] for key in sorted(ledger_by_id)),
            ],
        },
    )

    synthesis_path = map_root / "cluster_syntheses.yml"
    synthesis_payload = read_yaml(synthesis_path, {}) or {}
    syntheses = dict(synthesis_payload.get("syntheses", {}) or {})
    for cluster in clusters:
        cluster_id = str(cluster["cluster_id"])
    ...
    paths = [registry_path, synthesis_path]
    ...
    *,
    ...
    if analytical_unclustered:
        coverage_lines.extend(
            [
                "",
                "**Currently unclustered**",
                (
                    "These sources have no active cluster membership in this map "
                    "revision. They remain eligible for future plans as the library grows."
                ),
            ]
        )
        for row in analytical_unclustered:
            ...
            coverage_lines.append(f"- {_obsidian_note_link(source)}")
...
def persist_literature_report(
    ...
    *,
    ...
    for cluster in clusters:
        ...
        if cluster_publishable and not cluster.get("refresh_pending"):
    ...
    for cluster in clusters:
        ...
        if (
            ...
            and not cluster.get("refresh_pending")
        ):
            _write_managed_cluster_projection(
                workspace,
                canonical_cluster_path,
    ...
    *,
    ...
    if reasoner_calls is not None:
        report["manifest"].update(
            {
                "synthesis_call_count": reasoner_calls.cumulative_provider_calls,
                "synthesis_new_call_count": reasoner_calls.provider_calls,
