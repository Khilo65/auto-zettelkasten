...
""",
    ...
    assert read_yaml(tmp_path / "auto-zettelkasten.yml")["artifact_schema_version"] == "1.11"
...
""",
    ...
    assert note_path.read_bytes() == before
    assert read_yaml(tmp_path / "auto-zettelkasten.yml")["artifact_schema_version"] == "1.11"
    assert read_yaml(tmp_path / "11_state" / "workspace_manifest.yml")["artifact_schema_version"] == "1.11"
...
def test_schema_1_4_to_1_5_apply_is_local_byte_preserving_and_idempotent(
    ...
    assert read_yaml(tmp_path / "auto-zettelkasten.yml")["artifact_schema_version"] == "1.11"
    assert read_yaml(tmp_path / "11_state" / "workspace_manifest.yml")["engine_version"] == "0.12.0"
    assert read_yaml(tmp_path / "11_state" / "workspace_manifest.yml")["artifact_schema_version"] == "1.11"
    assert all(path.read_bytes() == content for path, content in preserved.items())
